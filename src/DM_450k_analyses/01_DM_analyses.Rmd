...
manuscriptDir <- file.path("docs", "manuscript", "data", "200128")
...
```{r output directory}
outputDir <- file.path("output", "01_DM_450k_analyses", "200120")
...
We liftover the coordinates to GRCh38 to update it.
...
Do the regions of differential methylation occur in any enhancers of particular genes?
...
Only SPI1 appears to display enhancer functions, but according to Ensembl Regulatory in Erythroblasts, Neutrophils and type I and II macrophages only.

No overlaps were found, but Javierre et al. might not be the only source to look at. We will next Try Ensembl Regulatory. 
...
set.seed(243098)
mvals_avr_glmnetcv <- cv.glmnet(x = t(mvals_avr_dmeg), y = as.numeric(pdata_avr$Severity), family = "binomial", type.measure = "mse", alpha = 0)

predict(mvals_avr_glmnetcv, newx = t(mvals_avr_dmeg), s = "lambda.min", type = "class")
```
...
CDvNon_CD_ma_th$Gene <- factor(CDvNon_CD_ma_th$Gene, levels = rev(CDvNon_CD_ma_th$Gene))
CDvNon_CD_ma_th$Significance <- "Non-significant"
CDvNon_CD_ma_th$Significance[CDvNon_CD_ma_th$Brown_padj<0.05] <- "Significant"

Cairo(width = 1000, height = 1500, type = "pdf", file = file.path(fig1Dir, "fig1b.pdf"), bg = "white", units = "px", dpi = 120)
