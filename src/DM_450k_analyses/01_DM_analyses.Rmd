...
nnat_plot <- dmr_plot(dmr_gr = nnat_gr, 
                      meth_data = betas_filtered, 
                      anno_gr = gmset_anno_gr, 
                      meth_groups = pdata_filtered$Severity, 
                      flanks = 0) +
  labs(title = "NNAT/BLCAP", 
       subtitle = "chr20:36148133-36151683")
...
mpig6b_plot <- dmr_plot(dmr_gr = mpig6b_gr, 
                        meth_data = betas_filtered, 
                        anno_gr = gmset_anno_gr, 
                        meth_groups = pdata_filtered$Severity, 
                        flanks = 0) +
  labs(title = "C6orf25/MPIG6B", 
       subtitle = "chr6:31689996-31692808")
...
hoxc4_plot <- dmr_plot(dmr_gr = hoxc4_gr, 
                        meth_data = betas_filtered, 
                        anno_gr = gmset_anno_gr, 
                        meth_groups = pdata_filtered$Severity, 
                        flanks = 0) +
...
```{r}
goi_gse73788 <- gse73788[grep("NNAT", gse73788$Gene),]
goi_gse73788$Cpgpos <- goi_gse73788$start+3
goi_gse73788_df <- goi_gse73788[,c("Mo1", "Mo2", "Mo3", "Cpgpos")]

goi_gse73788_melt <- reshape2::melt(goi_gse73788_df, id.vars = c("Cpgpos"))

ggplot(goi_gse73788_melt, aes(x = Cpgpos, y = value, col = variable)) +
  geom_point() +
  geom_smooth() +
  theme_bw()
...
#B
Cairo(width = 1500, height = 750, type = "pdf", file = file.path(fig3Dir, "fig3b.pdf"), bg = "white", units = "px", dpi = 120)
print(nnat_plot)
dev.off()

#C
Cairo(width = 1500, height = 750, type = "pdf", file = file.path(fig3Dir, "fig3c.pdf"), bg = "white", units = "px", dpi = 120)
print(mpig6b_plot)
dev.off()

#D
Cairo(width = 1500, height = 750, type = "pdf", file = file.path(fig3Dir, "fig3d.pdf"), bg = "white", units = "px", dpi = 120)
print(hoxc4_plot)
dev.off()
