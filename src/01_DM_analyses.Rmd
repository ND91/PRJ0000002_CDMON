...
#dmr_plotter is a function which plots the regions of interest within a region frame
dmr_plotter <- function(name, chr, betas, sample_names, annotation.gr, pheno, start_dmr, end_dmr, simplified = T, flanks = 1000, genome_version = "hg19", pdf = F){
  require(Gviz)
  ...
  dmr.hg19.annotation <- annotation.gr[which(seqnames(annotation.gr) == chr),]
  dmr.hg19.annotation <- dmr.hg19.annotation[which(start(dmr.hg19.annotation) >= start_dmr_flanks & start(dmr.hg19.annotation) <= end_dmr_flanks),]
  ...
  colnames(dmr.450k) <- c("seqnames", "pos", sample_names)
  ...
  dtrack.450k <- DataTrack(range = dmr.450k.gr, name = "450K", ylim = c(0,1), groups = as.factor(pheno), type = c("a", "p", "g"), legend = TRUE)
...
ind_cpg_plotter(cpg_num = "cg11267810", betas = Beta.culled, factor_interest = factor_interest)
...
ind_cpg_plotter(cpg_num = "cg11664417", betas = Beta.culled, factor_interest = factor_interest)
...
#HLA-J
dmr_plotter(name = "HLA-J", chr = "chr6", betas = Beta.rmout, sample_names = pData(GMset.rmout)$inclusionnr, annotation.gr = annotation.culled.gr, pheno = factor_interest.rmout, start_dmr = 29895175, end_dmr = 29895260, simplified = T)
#MOV10L1
dmr_plotter(name = "MOV10L1", chr = "chr22", betas = Beta.rmout, sample_names = pData(GMset.rmout)$inclusionnr, annotation.gr = annotation.culled.gr, pheno = factor_interest.rmout, start_dmr = 50528213, end_dmr = 50528312, simplified = T)
...
for(i in 1:nrow(sig.CD.ACT.top)){
  ...
  ind_cpg_plotter(cpg_num = rownames(sig.CD.ACT.top)[i], betas = Beta.CD, factor_interest = CD.state)
  dev.off()
}
#In case the HCs should be added
for(i in 1:nrow(sig.CD.ACT.top)){
  svg(filename = paste0(i, ".", rownames(sig.CD.ACT.top)[i], ".svg"))
  ind_cpg_plotter(cpg_num = rownames(sig.CD.ACT.top)[i], betas = Beta.rmout, factor_interest = factor_interest.rmout)
  dev.off()
}
...
for(i in 1:nrow(sig.CD.aTNF.top)){
  ...
  ind_cpg_plotter(cpg_num = rownames(sig.CD.aTNF.top)[i], betas = Beta.CD, factor_interest = pheno.CD$medicationrelevant)
  dev.off()
}
...
for(i in 1:nrow(sig.CD.ACTaTNF.top)){
  ...
  ind_cpg_plotter(cpg_num = rownames(sig.CD.ACTaTNF.top)[i], betas = Beta.rmout, factor_interest = strat.HC.CD.aTNF)
  dev.off()
}
```
