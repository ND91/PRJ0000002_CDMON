...
#ind_cpg_plotter produces a boxplot overlayed by a stripplot of a specific CpG
ind_cpg_plotter <- function(cpg_num, betas, factor_interest, gg.plot = T){
  require(ggplot2)
  require(gridExtra)
  
  if(is.null(cpg_num)){
    message("No CpG number defined")
  } else if(is.null(betas)){
    message("No Beta matrix provided")
  } else if(is.null(factor_interest)){
    message("No factor of interest provided")
  }
  ...
  if(gg.plot == T){
    small_plot <- ggplot(data = beta.df, aes(x = cohort, y = beta)) + 
      ...
      geom_jitter(size = 3, aes(shape = cohort)) + 
      theme_bw() + 
      xlab("")
    large_plot <- ggplot(data = beta.df, aes(x = cohort, y = beta)) + 
      ...
      xlab("")
    grid.arrange(small_plot, large_plot, ncol = 2, top = cpg_num)
  } else{
    boxplot(beta~cohort, data = beta.df, main = cpg_num, ylim = c(0,1), ylab = "Beta")
...
#ind_cpg_plotter(cpg_num = rownames(top.bin)[1], betas = Beta.culled, factor_interest = factor_interest)
...
ind_cpg_plotter(cpg_num = "cg11267810", betas = Beta.culled, factor_interest = factor_interest.bin)
...
ind_cpg_plotter(cpg_num = rownames(top.CDREM_CDACT)[1], betas = Beta.culled, factor_interest = factor_interest)
...
ind_cpg_plotter(cpg_num = rownames(top.CDREM_HC)[1], betas = Beta.culled, factor_interest = factor_interest)
...
ind_cpg_plotter(cpg_num = rownames(top.CDACT_HC)[1], betas = Beta.culled, factor_interest = factor_interest)
...
#ind_cpg_plotter(cpg_num = rownames(sva.top.CDREM_CDACT)[1], betas = Beta.culled, factor_interest = factor_interest)
...
#ind_cpg_plotter(cpg_num = rownames(sva.top.CDREM_HC)[1], betas = Beta.culled, factor_interest = factor_interest)
...
#ind_cpg_plotter(cpg_num = rownames(sva.top.CDACT_HC)[1], betas = Beta.culled, factor_interest = factor_interest)

#cg08534055
ind_cpg_plotter(cpg_num = "cg08534055", betas = Beta.culled, factor_interest = factor_interest)
ind_cpg_plotter(cpg_num = "cg08534055", betas = Beta.rmout, factor_interest = factor_interest.rmout)
...
factor_interest.rmout <- factor_interest[-out.index]
factor_interest.bin.rmout <- factor_interest.bin[-out.index]
```
...
#ind_cpg_plotter(cpg_num = rownames(top.rmout.bin)[1], betas = Beta.rmout, factor_interest = factor_interest.bin.rmout)
...
#Volcano plot of adjusted (moderated) P-values
mypar(1,1)
volplot <- data.frame(Coef = sva.fit2.rmout.bin.eb$coefficients[,2], Padj = sva.top.rmout.bin[rownames(sva.fit2.rmout.bin.eb),]$adj.P.Val)
x.lim <- c(-max(range(volplot$Coef)), max(range(volplot$Coef)))
if(-log10(0.05) > range(-log10(volplot$Padj))[2]+1){
  y.lim <- c(0,-log10(0.05))
} else{
...
abline(h = -log10(0.05), lty = 1)
```
Similar as when HC1 was left in, only SVA generates a significant hit, namely cg11267810. Plotting the CpG cg11267810 shows an interesting DMP, which is unfortunately still not significant at \alpha = 0.05. While the effect size is decent-ish (beta ~ 0.10), there appears to be some overlap between CD and HC, where the two lowest methylated CpGs CD fall within the distribution of the HCs. Similarly, the highest methylated HC falls overlaps with the median of CD. The unequal sampling of CD (17) versus HC (7) does not help either. 

```{r HC1-out: CD vs HC SVA, echo = F}
#All significant CpGs
sva.sig.rmout <- sva.top.rmout.bin[which(sva.top.rmout.bin$adj.P.Val < 0.05),]
...
for(i in 1:nrow(sva.sig.rmout)){
  svg(filename = paste0(getwd(), "/CD_HC_SVA/", rownames(sva.sig.rmout)[i]))
  ind_cpg_plotter(cpg_num = rownames(sva.sig.rmout)[i], betas = Beta.rmout, factor_interest = factor_interest.bin.rmout)
  #ind_cpg_plotter(cpg_num = rownames(sva.sig.rmout)[i], betas = Beta.rmout, factor_interest = factor_interest.rmout)
  dev.off()
}
```

One hit that was significant in the SVA analysis of the all samples and is still significant when removing HC1 is cg11267810. Interestingly, this CpG has not been associated to any gene. Another hit that was interesting was cg11664417, but two outliers in CD and HC confounds the data. 
```{r DMP: cg11267810, echo = F}
ind_cpg_plotter(cpg_num = "cg11267810", betas = Beta.rmout, factor_interest = factor_interest.bin.rmout)
ind_cpg_plotter(cpg_num = "cg11267810", betas = Beta.rmout, factor_interest = factor_interest.rmout)
annotation.total["cg11267810",]

ind_cpg_plotter(cpg_num = "cg11664417", betas = Beta.rmout, factor_interest = factor_interest.bin.rmout)
ind_cpg_plotter(cpg_num = "cg11664417", betas = Beta.rmout, factor_interest = factor_interest.rmout)
annotation.total["cg11664417",]
```
Stratifying the plots based on active or remissive CD, reveals that the outliers found in CD are located in CDACT and CDREM. The plots show that the CpG indeed appears to be differential in HC versus CD (both CDACT and CDREM).
...
#ind_cpg_plotter(cpg_num = rownames(top.rmout.CDREM_CDACT)[1], betas = Beta.rmout, factor_interest = factor_interest.rmout)
...
#ind_cpg_plotter(cpg_num = rownames(top.rmout.CDREM_HC)[1], betas = Beta.rmout, factor_interest = factor_interest.rmout)
...
#ind_cpg_plotter(cpg_num = rownames(top.rmout.CDACT_HC)[1], betas = Beta.rmout, factor_interest = factor_interest.rmout)
...
#ind_cpg_plotter(cpg_num = rownames(sva.rmout.CDREM_CDACT)[1], betas = Beta.culled, factor_interest = factor_interest)
...
#ind_cpg_plotter(cpg_num = rownames(sva.rmout.CDREM_HC)[1], betas = Beta.culled, factor_interest = factor_interest)
...
ind_cpg_plotter(cpg_num = "cg16921727", betas = Beta.rmout, factor_interest = factor_interest.rmout)
...
for(i in 1:nrow(hypdriv.sig)){
  svg(filename = paste0(i, ".", rownames(hypdriv.sig)[i]))
  ind_cpg_plotter(cpg_num = rownames(hypdriv.sig)[i], betas = Beta.rmout, factor_interest = factor_interest.rmout)
  dev.off()
}
...
ind_cpg_plotter("cg25721040", betas = Beta.CD, factor_interest = CD.state)
ind_cpg_plotter("cg13885357", betas = Beta.CD, factor_interest = CD.state)
...
ind_cpg_plotter(cpg_num = "cg25721040", betas = Beta.CD, factor_interest = pheno.CD$medicationrelevant)
ind_cpg_plotter(cpg_num = "cg13885357", betas = Beta.CD, factor_interest = pheno.CD$medicationrelevant)
...
ind_cpg_plotter(cpg_num = "cg09175811", betas = Beta.CD, factor_interest = pheno.CD$INCLgroup)
ind_cpg_plotter(cpg_num = "cg09175811", betas = Beta.CD, factor_interest = pheno.CD$medicationrelevant)

top.CD.aTNF.beta <- Beta.CD[c("cg25721040", "cg13885357", "cg09175811"),]
colnames(top.CD.aTNF.beta) <- pheno.CD$inclusionnr

```
There appear to be some significant hits, but closer analysis using the dotboxplot shows that the means are driven by two outliers, for ACT vs REM and aTNF vs non-aTNF the outliers appear to be CDREM-5 and CDREM-8, while the significant hits based on the interaction term appears to yield another two outliers, namely CDACT-2 and CDACT-7. Comparison of the medians does not yield any significant difference.

```{r CDREM-5 and CDREM-8, echo = F}
pData(GMset.culled)[c("3998634004_R03C01", "3998590010_R02C02"),]
View(pData(GMset.culled))
```
No clear correlation could be found between CDREM-5 and CDREM-8. Hierarchical clustering had not clustered them together.
...
for(i in 1:20){
  ...
  ind_cpg_plotter(cpg_num = rownames(top.TNF)[i], betas = Beta.CD, factor_interest = CD.aTNF)
  dev.off()
}
